...
--ensure ownership on these objects e.g. GRANT OWNERSHIP ON MASKING POLICY PII_MASKING_POLICY_DATE TO ROLE EMH_ETL_ROLE;
...
LET count NUMBER(38,0) := (select COUNT(*) 
                            ...
                            WHERE POLICY_NAME = 'PII_MASKING_POLICY_TIMESTAMP');
IF (count > 0) THEN
ALTER TAG PROTECTED_PII UNSET MASKING POLICY PII_MASKING_POLICY_TIMESTAMP;
END IF;
END;
$$
;
...
CREATE OR REPLACE MASKING POLICY PII_MASKING_POLICY_TIMESTAMP
...
ALTER TAG PROTECTED_PII SET MASKING POLICY PII_MASKING_POLICY_TIMESTAMP;
